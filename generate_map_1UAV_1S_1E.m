...
filename = 'map3.mat'; % Đặt tên file
save(filename, 'E', 'E_safe', 'E3d', 'E3d_safe', 'sizeE', 'd_grid', 'StartPoint', 'EndPoint', 'n_low');
...
%Dọn dẹp quanh vùng Start và End
x_range_0 = max(1, P0(2)-n_low) : min(x_size, P0(2)+n_low);
y_range_0 = max(1, P0(1)-n_low) : min(y_size, P0(1)+n_low);
E(y_range_0, x_range_0) = 0;

x_range_end = max(1, Pend(2)-n_low) : min(x_size, Pend(2)+n_low);
y_range_end = max(1, Pend(1)-n_low) : min(y_size, Pend(1)+n_low);
E(y_range_end, x_range_end) = 0;
...
% đảm bảo giữ vùng start/end là bằng không
E_safe(y_range_0, x_range_0) = 0;
E_safe(y_range_end, x_range_end) = 0;
